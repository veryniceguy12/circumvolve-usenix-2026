...
def generate_dns_request(target_domain=None):
    ...
    subtle irregularities to stress DPI while remaining acceptable to real
    resolvers:

      * QDCOUNT = 2 with a normal first question and a compressed second one.
      * The second question uses a *different* QTYPE (AAAA) to make the two
        questions non‑identical, which can exercise more parser code paths.
      * We keep the packet structure simple to preserve the good fitness score
    ...
    # ---- First Question (normal, noisy‑case QNAME, type A) ----
    qname1 = _encode_qname_with_noise(target_domain)
    qtype1 = 1    # A
    qclass1 = 1   # IN
    question1 = qname1 + struct.pack("!HH", qtype1, qclass1)

    # ---- Second Question (compressed name, type AAAA) ----
    ...
    qtype2 = 28   # AAAA
    qclass2 = 1   # IN
    question2 = compressed_name + struct.pack("!HH", qtype2, qclass2)
