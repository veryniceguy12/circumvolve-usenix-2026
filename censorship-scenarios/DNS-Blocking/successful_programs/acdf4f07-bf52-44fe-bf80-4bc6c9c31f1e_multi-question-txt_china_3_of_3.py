...
def generate_dns_request(target_domain=None):
    """
    Generates a DNS request payload that is RFC-compliant but slightly
    unusual in flag bits and QTYPE mix to explore different DPI behaviors,
    while preserving the successful two-question pattern.
    """
    ...
    #
    # We still keep RD=1 for compatibility, but also set the CD
    # (Checking Disabled) bit to 1, which is valid yet less common
    # in many client queries and may exercise different DPI paths.
    #
    # Desired: QR=0, Opcode=0, AA=0, TC=0, RD=1, RA=0, Z=0, AD=0, CD=1, RCODE=0
    # Binary: 0000 0001 0000 0010 -> 0x0102
    flags = 0x0102

    # Two questions on the same QNAME keep the already successful structure,
    # but we change the second QTYPE from AAAA to TXT to diversify behavior.
    qdcount = 2
    ancount = 0
    nscount = 0
    ...
    # First question: type A, class IN (common and safe)
    question1 = qname + struct.pack("!HH", 0x0001, 0x0001)

    # Second question: type TXT, class IN (valid but less typical pairing with A)
