#!/usr/bin/env python3
"""
Run DNS censorship evasion evaluation across multiple countries.
...
def main(input_args=None):
    parser = argparse.ArgumentParser(
        description="Run country-wise DNS censorship evasion evaluation",
...
"""
    ...
    for entry in countries:
        ...
        censor_type = entry.get("censor_type", "in-path-modifier")
        ...
        print(f"  Forbidden Domain: {entry.get('forbidden_domain', 'N/A')}")
        print("=" * 70)
        ...
        # Enable cascade evaluation for on-path-injector
        if censor_type == "on-path-injector":
            if "evaluator" not in country_config:
                country_config["evaluator"] = {}
            country_config["evaluator"]["cascade_evaluation"] = True
            country_config["evaluator"]["cascade_thresholds"] = [1.0]  # Stage 1 needs at least 1 resolver success
            print(f"  Cascade evaluation ENABLED (required for on-path-injector)")
        ...
        if "non_dns_server" in entry:
            remote_settings["non_dns_server"] = entry["non_dns_server"]
        ...
        # Add expected resolved IP for Stage 1 anti-cheat validation
        if "expected_resolved_ip" in entry:
            country_config["scenario_config"]["expected_resolved_ip"] = entry["expected_resolved_ip"]
            print(f"  Expected resolved IP: {entry['expected_resolved_ip']} (required for Stage 1)")
        ...
        # Copy dns_initial.py to output directory
        dns_initial_src = os.path.join(base_dir, "dns_initial.py")
        dns_initial_dst = os.path.join(country_dir, "dns_initial.py")
        if os.path.exists(dns_initial_src):
            shutil.copy2(dns_initial_src, dns_initial_dst)
            print(f"  Copied dns_initial.py to {dns_initial_dst}")

            # Replace 'blocked.com' with the forbidden_domain
            forbidden_domain = country_config.get("scenario_config", {}).get("forbidden_domain")
            if forbidden_domain:
                try:
                    with open(dns_initial_dst, 'r', encoding='utf-8') as f:
                        content = f.read()
                    
                    if 'blocked.com' in content:
                        new_content = content.replace('blocked.com', forbidden_domain)
                        with open(dns_initial_dst, 'w', encoding='utf-8') as f:
                            f.write(new_content)
                        print(f"  Replaced 'blocked.com' with '{forbidden_domain}' in {dns_initial_dst}")
                except Exception as e:
                    print(f"  Warning: Failed to replace domain in {dns_initial_dst}: {e}")
        else:
            print(f"  Warning: {dns_initial_src} not found, could not copy.")
            continue
