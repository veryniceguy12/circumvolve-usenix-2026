# EVOLVE-BLOCK-START
"""
Kazakhstan: Desync - load on second SYN
Geneva: [TCP:flags:S]-duplicate(,tamper{TCP:load:corrupt})-|
Success rate: 100% against Kazakhstan censors
"""
...
class Strategy:
    ...
    def on_outgoing_packet(self, pkt: Packet) -> Optional[List[Union[Packet, Scheduled]]]:
        ...
        """
        if not pkt.haslayer(TCP) or pkt[TCP].flags != 'S':
            return [pkt]
        
        corrupted_syn = copy.deepcopy(pkt)
        # Add corrupted payload to SYN
