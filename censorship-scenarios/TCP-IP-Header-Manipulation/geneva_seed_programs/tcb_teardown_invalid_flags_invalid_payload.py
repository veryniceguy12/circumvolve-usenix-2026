# EVOLVE-BLOCK-START
"""
TCB Teardown - Invalid Flags - Invalid Payload
Geneva: [TCP:flags:A]-tamper{TCP:load:replace:a5z2}(tamper{TCP:flags:replace:CN},)-
Success rate: 0% against China GFW (non-functional reference)
"""
...
class Strategy:
    ...
    def on_outgoing_packet(self, pkt: Packet) -> Optional[List[Union[Packet, Scheduled]]]:
        ...
        modified[TCP].flags = 'CN'
        if not modified.haslayer(Raw):
            modified = modified / Raw(load=b'a5z2')
        else:
            modified[Raw].load = b'a5z2'
