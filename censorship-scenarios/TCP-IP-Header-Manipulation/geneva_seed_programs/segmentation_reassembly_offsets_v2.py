# EVOLVE-BLOCK-START
"""
Segmentation - Reassembly - Offsets, subvariant 2
Geneva: [TCP:flags:PA]-fragment{tcp:4:True}(,fragment{tcp:19:True})-
...
class Strategy:
    ...
    def on_outgoing_packet(self, pkt: Packet) -> Optional[List[Union[Packet, Scheduled]]]:
        ...
        # First fragment: first 4 bytes
        pkt1 = copy.deepcopy(pkt)
        pkt1[Raw].load = payload[:4]
        pkt1 = recalculate_checksums(pkt1)
        
        # Second fragment: bytes 19-end
        pkt2 = copy.deepcopy(pkt)
        pkt2[TCP].seq = seq + 19
        pkt2[Raw].load = payload[19:]
