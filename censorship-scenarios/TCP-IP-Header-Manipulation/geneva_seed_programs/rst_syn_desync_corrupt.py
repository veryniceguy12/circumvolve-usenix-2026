# EVOLVE-BLOCK-START
r"""
Lab: RST + SYN desynchronization with corrupt seq
Geneva: [TCP:flags:A]-duplicate(tamper{TCP:flags:replace:R}(tamper{TCP:chksum:replace:15239},),
                        duplicate(tamper{TCP:flags:replace:S}(tamper{TCP:seq:corrupt}(tamper{TCP:chksum:corrupt},),),))-| \/

Similar to above but with corrupted sequence number on SYN for additional desync.

Evades: censor1, censor2, censor3, censor5, censor6, censor7, censor8, censor8b, censor9
"""
...
class Strategy:
    ...
    def on_outgoing_packet(self, pkt: Packet) -> Optional[List[Union[Packet, Scheduled]]]:
        ...
        rst_pkt[TCP].flags = 'R'
        rst_pkt[TCP].chksum = 15239
        
        syn_pkt = copy.deepcopy(pkt)
        syn_pkt[TCP].flags = 'S'
        syn_pkt[TCP].seq = random.randint(0, 65535)
        syn_pkt[TCP].chksum = random.randint(0, 65535)
        
        return [rst_pkt, syn_pkt, pkt]
