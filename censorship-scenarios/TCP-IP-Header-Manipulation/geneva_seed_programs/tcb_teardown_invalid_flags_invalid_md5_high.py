# EVOLVE-BLOCK-START
"""
TCB Teardown - Invalid Flags - Invalid md5Header, High Percentage
Geneva: [TCP:flags:A]-duplicate(,tamper{TCP:flags:replace:FRAPUN}(tamper{TCP:options-md5header:corrupt},))-
...
class Strategy:
    ...
    def on_outgoing_packet(self, pkt: Packet) -> Optional[List[Union[Packet, Scheduled]]]:
        ...
        invalid = copy.deepcopy(pkt)
        invalid[TCP].flags = 'FRAPUN'
        invalid[TCP].options = [('MD5', bytes([random.randint(0, 255) for _ in range(16)]))]
        invalid = recalculate_checksums(invalid)
