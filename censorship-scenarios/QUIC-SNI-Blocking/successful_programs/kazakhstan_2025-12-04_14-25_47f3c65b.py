...
"""
ACK PREFIX
Strategy: switch from payload/frame weirdness (which produced malformed packets)
to *header+varint anomalies* while keeping the QUIC+TLS crypto structurally valid.
We still send a standard ClientHello with SNI=pornhub.com, but we:
(1) put CRYPTO at offset 0 in a single frame (simpler for servers),
(2) add an ACK frame with bizarre but syntactically valid varints before CRYPTO,
(3) pad with many PADDING bytes to 1200 and rely on odd ACK ranges to confuse DPI.
"""
...
class QUICCrypto:
    ...
    def __init__(self, dcid: bytes, version: int):
        ...
        self.client_key = self._hkdf_expand_label(
            client_initial_secret, b"quic key", b"", self.aead_key_length
        )
        self.client_iv = self._hkdf_expand_label(
            client_initial_secret, b"quic iv", b"", self.iv_length
        )
        self.client_hp_key = self._hkdf_expand_label(
            client_initial_secret, b"quic hp", b"", self.hp_key_length
        )

        # Derive server keys
        self.server_key = self._hkdf_expand_label(
            server_initial_secret, b"quic key", b"", self.aead_key_length
        )
        self.server_iv = self._hkdf_expand_label(
            server_initial_secret, b"quic iv", b"", self.iv_length
        )
        self.server_hp_key = self._hkdf_expand_label(
            server_initial_secret, b"quic hp", b"", self.hp_key_length
        )
...
def build_default_initial_payload(scid: bytes) -> bytes:
    """
    Construct a QUIC Initial payload that is syntactically close to normal
    but uses header/varint anomalies rather than weird CRYPTO layout:
      * An ACK frame with very large packet numbers and gaps (all before any
        real packets), to stress DPI state tracking but be safely ignorable.
      * A single CRYPTO frame at offset 0 carrying the full ClientHello,
        making the crypto stream straightforward for the server.
      * PADDING to push the overall size to 1200 bytes (done by outer logic).
    """
    client_hello = _build_tls_client_hello(scid)

    # 1) ACK frame with pathological but valid varints.
    # Frame type 0x02 (ACK). Use 2 ack ranges and a huge largest_acked.
    # Layout (QUIC v1):
    #   type (0x02)
    ...
    #   repeated: gap (varint), ack_range_len (varint) [for each extra range]
    ack_ft = b"\x02"
    largest_acked = encode_varint(10_000_000)   # absurdly high PN
    ack_delay = encode_varint(0)                # no delay
    ack_range_count = encode_varint(1)          # two ranges total
    first_ack_range = encode_varint(0)          # just that PN
    gap1 = encode_varint(2)                     # gap then
    ack_range_len1 = encode_varint(0)           # single PN again
    ack_frame = (
        ack_ft
        + largest_acked
        + ack_delay
    ...
    # 2) Single CRYPTO frame at offset 0 containing full ClientHello.
    frame_type_crypto = b"\x06"
    offset = encode_varint(0)
    crypto_len = encode_varint(len(client_hello))
    crypto_frame = frame_type_crypto + offset + crypto_len + client_hello

    # 3) Some front PADDING to move actual TLS bytes deeper.
    front_pad_len = 64
    front_padding = b"\x00" * front_pad_len

    return front_padding + ack_frame + crypto_frame
...
def build_packet(config: PacketConfig) -> Tuple[bytes, bytes, bytes]:
    ...
    if len(config.packet_type) != 2 or any(b not in '01' for b in config.packet_type):
        raise ValueError("packet_type must be a 2-bit string, e.g. '00', '01', '10', '11'.")
    pt = int(config.packet_type, 2)
