...
"""
BOGUS CRYPTO FRAME
Strategy: move away from the “empty CRYPTO at offset 0 + real CH at high offset”
layout (which only gives evasion_success≈0.1) and instead abuse CRYPTO offsets in
the opposite order: send a bogus CRYPTO frame at a *large* offset filled with zeros,
then PADDING, then a CRYPTO frame at offset 0 that carries the real ClientHello.
This matches the best‑known high‑fitness shape (Program 1) while keeping code size
small, and may better confuse DPIs that key off the first or highest‑offset CRYPTO.
"""
...
def build_default_initial_payload(scid: bytes) -> bytes:
    """
    Construct a QUIC Initial payload that mirrors the highest‑fitness known
    layout but with a shorter, simpler implementation:

    - First, a bogus CRYPTO frame at a large non‑zero offset carrying only zeros.
    - Then a block of PADDING bytes.
    - Finally, a CRYPTO frame at offset 0 with the full TLS 1.3 ClientHello
      (with SNI intact).

    Standards‑compliant endpoints reconstruct the crypto stream by offset and
    observe a normal ClientHello starting at 0. Simpler DPI that (a) inspects
    only the first CRYPTO frame in the packet or (b) keys off the highest offset
    may see only zeros or reject the packet prematurely, improving evasion.
    """
    client_hello = _build_tls_client_hello(scid)
    frame_type_crypto = b"\x06"

    # Bogus CRYPTO frame at a high offset with zero data
    bogus_offset = encode_varint(4096)
    bogus_len = encode_varint(128)
    bogus_payload = b"\x00" * 128
    bogus_crypto_frame = frame_type_crypto + bogus_offset + bogus_len + bogus_payload

    # PADDING to move the real CH further into the packet
    padding_block = b"\x00" * 400

    # Real CRYPTO frame at offset 0 with the full ClientHello
    real_offset = encode_varint(0)
    real_len = encode_varint(len(client_hello))
    real_crypto_frame = frame_type_crypto + real_offset + real_len + client_hello

    return bogus_crypto_frame + padding_block + real_crypto_frame
