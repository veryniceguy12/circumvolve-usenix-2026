...
"""
DUAL CLIENT HELLO
Strategy: switch from "single bloated CH + extra frames" to "dual-ClientHello with split SNI".
We now build two TLS ClientHello messages: a full-featured *no-SNI* CH carried in one CRYPTO frame,
and a tiny *SNI-only* CH for cloudflare.com carried in a second CRYPTO frame at a non-zero offset
with padding around them. This mimics the best high-fitness dual-CH designs while still looking QUIC-v1
canonical on the wire, forcing DPI to do deeper CRYPTO+TLS reassembly to see the forbidden SNI.
"""
...
def _build_tls_client_hello(scid: bytes) -> bytes:
    """
    Build two TLS 1.3 ClientHello handshake messages:
    - ch_main: a standard-looking ClientHello with full extensions but WITHOUT SNI.
    - ch_sni: a tiny ClientHello that carries ONLY SNI (cloudflare.com) plus minimal extensions.
    Concatenating them lets real servers complete from ch_main while DPI has to
    notice and parse an additional, unusual CH to see the forbidden SNI.
    """
    ...
    random_main = bytes(range(32))
    random_sni = bytes(reversed(range(32)))
    session_id = b""
    cipher_suites = [
    ...
    # ---------- Main ClientHello: full but NO SNI ----------
    ext_main = []

    # Supported Groups
    groups = [0x001d, 0x0017, 0x0018]
    group_bytes = b"".join(g.to_bytes(2, "big") for g in groups)
    ext_main.append(_ext(0x000A, len(group_bytes).to_bytes(2, "big") + group_bytes))

    # Signature Algorithms
    signature_algorithms = [0x0403, 0x0804, 0x0805, 0x0806, 0x0503, 0x0603]
    sig_alg_bytes = b"".join(sa.to_bytes(2, "big") for sa in signature_algorithms)
    ext_main.append(_ext(0x000D, len(sig_alg_bytes).to_bytes(2, "big") + sig_alg_bytes))

    # Supported Versions – TLS 1.3 plus a draft-ish version for slight oddity
    ext_main.append(_ext(0x002B, b"\x04\x03\x04\x7f\x17"))

    # PSK key exchange modes
    ext_main.append(_ext(0x002D, b"\x01\x01"))
    ...
    share_entry = 0x001D.to_bytes(2, "big") + len(key_exchange).to_bytes(2, "big") + key_exchange
    ext_main.append(_ext(0x0033, len(share_entry).to_bytes(2, "big") + share_entry))

    # ALPN HTTP/3
    alpn_protocols = b"\x02h3"
    ext_main.append(_ext(0x0010, len(alpn_protocols).to_bytes(2, "big") + alpn_protocols))

    # QUIC transport parameters
    transport_parameters = _build_quic_transport_parameters(scid)
    ext_main.append(_ext(0x0039, transport_parameters))

    # Padding-like extension to bloat ClientHello a bit
    ext_main.append(_ext(0x0015, b"\x00" * 96))

    # Unknown extension to preserve some structural weirdness
    ext_main.append(_ext(0xFAFA, b"\x00" * 8 + b"DUALCHLO"))

    ext_main_bytes = b"".join(ext_main)
    ext_main_len = len(ext_main_bytes).to_bytes(2, "big")
    ...
    ch_main = b"\x01" + len(ch_main_body).to_bytes(3, "big") + ch_main_body

    # ---------- Secondary ClientHello: minimal but WITH SNI ----------
    ext_sni = []

    host_name = "google.com"
    host_bytes = host_name.encode("ascii")
    server_name = b"\x00" + len(host_bytes).to_bytes(2, "big") + host_bytes
    server_name_list = len(server_name).to_bytes(2, "big") + server_name
    ext_sni.append(_ext(0x0000, server_name_list))

    # Minimal supported_versions to keep it syntactically valid
    ext_sni.append(_ext(0x002B, b"\x02\x03\x04"))

    # Tiny padding extension so CH is not trivially small
    ext_sni.append(_ext(0x0015, b"\x00" * 8))

    ext_sni_bytes = b"".join(ext_sni)
    ext_sni_len = len(ext_sni_bytes).to_bytes(2, "big")
    ...
    ch_sni = b"\x01" + len(ch_sni_body).to_bytes(3, "big") + ch_sni_body

    # main (no SNI) first, tiny SNI-only afterwards
    return ch_main + ch_sni


def build_default_initial_payload(scid: bytes) -> bytes:
    """
    Construct a QUIC Initial payload with two CRYPTO frames:
    - First CRYPTO at offset 0 carrying the main no-SNI ClientHello.
    - Second CRYPTO at a non-zero offset carrying the tiny SNI-only ClientHello.
    We also add PADDING before and between the CRYPTO frames so that:
    * CRYPTO is not at byte 0 of the Initial payload, and
    * the second CH (with SNI) is displaced, complicating naive DPI expectations.
    """
    ch_combined = _build_tls_client_hello(scid)

    # Split concatenated handshakes using the first handshake header
    main_len = int.from_bytes(ch_combined[1:4], "big") + 4
    ch_main = ch_combined[:main_len]
    ch_sni = ch_combined[main_len:]

    # Leading padding so first CRYPTO does not start at payload byte 0
    padding_prefix = b"\x00" * 12

    # First CRYPTO frame: offset 0, main ClientHello (no SNI)
    crypto1 = (
        b"\x06"
        + encode_varint(0)
        + encode_varint(len(ch_main))
        + ch_main
    )

    # Padding between CRYPTO frames to create a visible gap in offsets
    padding_between = b"\x00" * 10

    # Second CRYPTO frame: offset beyond first CH plus the explicit gap
    crypto2_offset = len(ch_main) + len(padding_between)
    crypto2 = (
        b"\x06"
        + encode_varint(crypto2_offset)
        + encode_varint(len(ch_sni))
        + ch_sni
    )

    # Trailing padding; outer builder will pad further to 1200 bytes
    padding_suffix = b"\x00" * 8

    return padding_prefix + crypto1 + padding_between + crypto2 + padding_suffix
