...
def _parse_port_range(port_range: str) -> Tuple[int, int]:
    """
    Parse a port range string (e.g., "443-5000") into start and end ports.
    
    Args:
        port_range: String in format "start-end" (e.g., "443-5000")
    ...
    try:
        ...
        if start_port < 1 or end_port > 65535 or start_port > end_port:
            raise ValueError(f"Invalid port range: {start_port}-{end_port}. Ports must be 1-65535 and start <= end.")
        return start_port, end_port
    except (ValueError, AttributeError) as exc:
        raise ValueError(f"Failed to parse port range '{port_range}': {exc}") from exc


def _select_random_port(port_range: Optional[str], default_port: int = 443) -> int:
    """
    Select a random port from a port range, or return the default port if no range is provided.
    
    Args:
        port_range: Optional string in format "start-end" (e.g., "443-5000")
        default_port: Default port to use if no range is provided
    ...
    try:
        ...
        logger.info("Selected random port %d from range %s", selected_port, port_range)
        return selected_port
    except ValueError as exc:
        logger.warning("Failed to parse port range, using default port %d: %s", default_port, exc)
        return default_port
...
def _evaluate_remote(
    ...
    Args:
        ...
        server_port: Target server port to test against (default: 443)
        program_id: Optional ID to pass to the evaluator
        censor_type: Type of censor - "in-path" (drops/blocks packets) or "on-path" (injects RST/forged packets)
    ...
    try:
        ...
        try:
            ...
            # Upload program
            remote_program_path = f"{remote_workspace}/quic_initial.py"
            ...
            cmd = (
                ...
                f"--quic-initial-path quic_initial.py "
...
def evaluate(
    ...
    Args:
        ...
        program_id: Optional ID to pass to the evaluator
        censor_type: Type of censor - "in-path" (drops/blocks) or "on-path" (injects RST/forged packets)

    Returns:
        EvaluationResult with metrics and artifacts
    """
    # Initialize server_port_range for later use
    ...
    if not remote_host:
        ...
        # Override censor_type from config if still default
        if censor_type == "in-path":
            censor_type = scenario_config.get("censor_type", censor_type)
        
        # Get server_port_range for unique port selection (useful for on-path censors)
        server_port_range = scenario_config.get("server_port_range")

    if remote_host:
        if not remote_user:
            raise ValueError("remote_user must be provided if remote_host is set (via args or config.yaml)")
        
        # Select a random port from the configured range (or use default 443)
        server_port = _select_random_port(server_port_range)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Type of censor: 'in-path' (drops/blocks packets) or 'on-path' (injects RST/forged packets) (default: in-path)",
    )
