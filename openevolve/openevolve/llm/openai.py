...
class OpenAILLM(LLMInterface):
    ...
    async def generate_with_context(
        ...
        is_anthropic_api = "anthropic" in str(self.api_base).lower() if self.api_base else False
        ...
        if is_openai_reasoning_model:
            ...
            # For Anthropic API, only send one of temperature or top_p (prefer temperature)
            if is_anthropic_api:
                temperature = kwargs.get("temperature", self.temperature)
                top_p = kwargs.get("top_p", self.top_p)
                # Prefer temperature if both are set, otherwise use whichever is set
                if temperature is not None:
                    params["temperature"] = temperature
                elif top_p is not None:
                    params["top_p"] = top_p
            else:
                # For other APIs, send both if configured
                temperature = kwargs.get("temperature", self.temperature)
                top_p = kwargs.get("top_p", self.top_p)
                if temperature is not None:
                    params["temperature"] = temperature
        ...
        use_max_completion_tokens = is_openai_reasoning_model
        ...
        for attempt in range(retries + 1):
            try:
                ...
                error_str = str(e)
                # Check if the error is about max_tokens not being supported
                # This can happen if a reasoning model wasn't detected initially
                if ("max_tokens" in error_str and "max_completion_tokens" in error_str.lower() 
                    and not use_max_completion_tokens):
                    logger.warning(
                        f"Model requires max_completion_tokens instead of max_tokens. "
                        f"Retrying with corrected parameter..."
                    )
                    # Convert params to use max_completion_tokens
                    ...
                    try:
                        ...
                        # If retry also fails, continue with normal retry logic
                        if attempt < retries:
                            logger.warning(
                                f"Error on attempt {attempt + 1}/{retries + 1}: {str(retry_e)}. Retrying..."
                            )
                            await asyncio.sleep(retry_delay)
                        else:
                            logger.error(f"All {retries + 1} attempts failed with error: {str(retry_e)}")
                            raise
                # Check if the error is about temperature and top_p both being specified
                # This can happen with Anthropic models or other APIs with similar restrictions
                elif ("temperature" in error_str.lower() and "top_p" in error_str.lower() 
                      and ("cannot both be specified" in error_str.lower() or "cannot both" in error_str.lower())
                      and "temperature" in params and "top_p" in params):
                    logger.warning(
                        f"Model does not allow both temperature and top_p. "
                        f"Retrying with only temperature..."
                    )
    ...
    async def _call_api(self, params: Dict[str, Any]) -> str:
        ...
        # Add this block to log token usage
