...
def _get_extension_for_language(language: str) -> str:
    ...
    return LANGUAGE_TO_EXTENSION.get(language.lower(), ".txt")
...
class ProgramDatabase:
    ...
    def sample_from_island(
        ...
        if not island_programs:
            # Island is empty - initialize it with a copy of best program
            # This ensures programs don't all accumulate in island 0 when using lazy seeding
            if self.best_program_id and self.best_program_id in self.programs:
                ...
                copy_program = Program(
                    ...
                    metadata={"island": island_id},  # Assign to THIS island
                ...
                self.islands[island_id].add(copy_program.id)
                logger.info(f"Initialized empty island {island_id} with copy of best program {best_program.id}")
                
                # Use the copy as the parent
                parent = copy_program
                inspirations = self._sample_inspirations(parent, n=num_inspirations or 5)
                return parent, inspirations
            else:
                # No best program yet, fall back to sampling from all programs
                # This should only happen at the very start before any evaluation
                logger.warning(f"Island {island_id} empty and no best program available, falling back to global sample")
    ...
    def _save_program(
        ...
        # Also save the program code as a separate file with the appropriate extension
        extension = _get_extension_for_language(program.language)
        code_path = os.path.join(programs_dir, f"{program.id}{extension}")
    ...
    def _calculate_feature_coords(self, program: Program) -> List[int]:
        ...
        for dim in self.config.feature_dimensions:
            ...
            # PRIORITY 2: Fall back to built-in features if not in metrics
                ...
                # Feature dimension not found in metrics - use default bin 0 (lowest)
                # This can happen when evaluation fails/times out and returns error metrics
                # Log a warning but don't crash - allow the program to be added to the database
                logger.warning(
                    f"Feature dimension '{dim}' not found in program metrics for program {program.id}. "
                    f"Available metrics: {list(program.metrics.keys())}. "
                    f"Using default bin 0 for this dimension. "
                    f"This typically happens when evaluation fails or times out."
                )
                coords.append(0)  # Default to lowest bin for failed/timed-out evaluations
