...
def run_evolution(
    ...
    cleanup: bool = True,
    seed_programs: Optional[List[Union[str, Path]]] = None,
) -> EvolutionResult:
    """
    ...
    Args:
        ...
        seed_programs: Optional list of paths to seed programs for islands.
            Each program seeds one island (first -> island 0, second -> island 1, etc.)
            If fewer seeds than islands, remaining islands get LLM-generated variations.
            If provided, initial_program is still required but seed_programs take precedence
            for island seeding.
    ...
    Examples:
        ...
        # Using multiple seed programs for islands
        result = run_evolution(
            initial_program='program.py',
            evaluator='evaluator.py',
            seed_programs=['seed_a.py', 'seed_b.py', 'seed_c.py'],
            iterations=100
        )
    """
    return asyncio.run(
        _run_evolution_async(
            initial_program, evaluator, config, iterations, output_dir, cleanup, seed_programs
        )
    )
...
async def _run_evolution_async(
    ...
    try:
        ...
        # Apply seed programs if provided
        if seed_programs:
            # Validate that all seed program files exist
            seed_paths = []
            for seed_path in seed_programs:
                seed_path_str = str(seed_path)
                if not os.path.exists(seed_path_str):
                    raise FileNotFoundError(f"Seed program file not found: {seed_path_str}")
                seed_paths.append(seed_path_str)
            config_obj.database.seed_programs_paths = seed_paths
